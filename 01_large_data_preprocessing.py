...
def aggregate_weekly(df):
    ...
    weekly = (
        df.groupby(["week", "Grondstof", "container"])
        .agg(
            liters =("Liter","sum"),
            Factuurnummer = ("Factuurnummer", lambda x: ", ".join(x.astype(str).unique())),
            Customer_Name = ("Commerciele_naam", lambda x: ", ".join(x.astype(str).unique()))
        )
        .reset_index()
    )
